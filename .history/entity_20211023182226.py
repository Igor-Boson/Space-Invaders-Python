...
class Alien(Entity):
    ...
    def __init__(self, handler, x, y, starting_speed=1.0, speed_increase_factor=1.0):
        ...
        self.movement_cap = (self.handler.screen.width - 2*self.handler.game.starting_x_value\
                            - self.handler.game.am_width*self.size - (self.handler.game.am_width - 1)*self.handler.game.x_value_increase)\
                            /(self.size + self.handler.game.x_value_increase)
        print(self.movement_cap)
    ...
    def move(self):
        ...
        # goes sideways
            self.x += self.handler.game.x_value_increase
            self.tick_counter = 0
